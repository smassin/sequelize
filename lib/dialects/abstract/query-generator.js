...
var QueryGenerator = {
  ...
  updateQuery: function(tableName, attrValueHash, where, options, attributes) {
    ...
    for (var key in attrValueHash) {
      ...
      if(value instanceof Utils.increment) {
        values.push(this.quoteIdentifier(key) + '=' + this.quoteIdentifier(key) + "+" + this.escape(value, (modelAttributeMap && modelAttributeMap[key] || undefined), { context: 'UPDATE' }));
      }
      else {
  ...
  */
  ...
  selectQuery: function(tableName, options, model) {
    ...
    if(options.with && !Utils._.isArray(options.with)) {
        ...
        if (include.include) {
          ...
          include.include.filter(function (include) {
            ...
            if (childInclude.subQuery && subQuery) {
              if(childInclude.dependent === true) {
                dependentQueries = dependentQueries.concat(childJoinQueries.subQuery)
              }
              else {
            ...
            if (childJoinQueries.mainQuery) {
              if(childInclude.dependent === true) {
                dependentQueries = dependentQueries.concat(childJoinQueries.mainQuery)
              }
              else {
          ...
          // if(dependentQueries.length) {
          //   joinQueryItem = joinQueryItem.replace(/JOIN (.*) ON/, "JOIN ($1 " + dependentQueries.join(' ') + ") ON")
          // }
          if(dependentQueries.length) {
            // splice original joinQueryItem string -- use string concatenation instead of regex to support through queries with subjoins like through queries
            var firstJoinIndex = joinQueryItem.indexOf('JOIN') + 5;
            var lastOnIndex = joinQueryItem.lastIndexOf(' ON');
            joinQueryItem = joinQueryItem.substring(0, firstJoinIndex) + "(" + joinQueryItem.substring(firstJoinIndex, lastOnIndex) + dependentQueries.join(' ') + ")" + joinQueryItem.substring(lastOnIndex);
          }
        }

        if (include.subQuery && subQuery) {
          joinQueries.subQuery = [joinQueryItem].concat(joinQueries.subQuery);
        } else {
          joinQueries.mainQuery = [joinQueryItem].concat(joinQueries.mainQuery);
        }
  ...
  joinIncludeQuery: function(options) {
    ...
    if(include.with) {
      withFragment += ' WITH (' + include.with.join(',') + ')';
    }

    return joinType + this.quoteTable(tableRight, asRight) + withFragment + ' ON ' + joinOn;
  },
  ...
  handleSequelizeMethod: function (smth, tableName, factory, options, prepend) {
    ...
    if (smth instanceof Utils.where) {
      ...
      result = smth.val;
    } else if (smth instanceof Utils.increment) {
      result = smth.val;
