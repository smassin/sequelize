...
var QueryGenerator = {
  ...
  upsertQuery: function(tableName, insertValues, updateValues, where, rawAttributes, options) {
    var query = 'MERGE INTO <%= tableNameQuoted %> ' + ( options.atomic === true ? 'WITH(HOLDLOCK)' : '' ) + ' AS <%= targetTableAlias %> USING (<%= sourceTableQuery %>) AS <%= sourceTableAlias%>(<%=insertKeysQuoted%>) ON <%= joinCondition %>';

    if(updateValues && Object.keys(updateValues).length)
      query += ' WHEN MATCHED THEN UPDATE SET <%= updateSnippet %>';
    query += ' WHEN NOT MATCHED THEN INSERT <%= insertSnippet %>';//' OUTPUT $action, INSERTED.*;';

  
    //To capture output rows when there is a trigger on MSSQL DB
    if (options.hasTrigger) {
        var tmpTable = 'declare @tmp table ([$action] nvarchar(6)); ';
        query = tmpTable + query;
        query += ' OUTPUT $action into @tmp; select * from @tmp;';
    }
    else {
      query += ' OUTPUT $action, INSERTED.*;';
    }


    var targetTableAlias = this.quoteTable(tableName + '')
    ...
    for (var key in rawAttributes) {
      ...
      if (rawAttributes[key].unique && (!options.unique || rawAttributes[key].unique == options.unique)) {
    ...
    var clauses = where.$or.filter(function(clause) {
      if(options.allowNull === true) {
        // This only works because we are not allowing more than one null in a unique index -- the above is actually more correct
        var valid = false;
    ...
    var getJoinSnippet = function(array) {
      return Utils._.map(array, function(key) {
        var keyed = this.quoteIdentifier(key);
        if(insertValues[key] === null) { // This allows comparing NULL columns
          return '(' + targetTableAlias + '.' + keyed + ' IS NULL AND ' + sourceTableAlias + '.' + keyed + ' IS NULL)';
        }
        return targetTableAlias + '.' + keyed + ' = ' + sourceTableAlias + '.' + keyed;
      }.bind(this));
    };

    if (clauses.length === 0) {
      throw new Error('Primary Key or Unique key should be passed to upsert query for ' + tableName);      
    } else {
    ...
    updateSnippet = Utils._.map(updateSnippet, function(key) {
      var value = updateValues[key];
      key = this.quoteIdentifier(key);
      var escapeValue = this.escape(value);
      if(value instanceof Utils.increment) {
        return targetTableAlias + '.' + key + ' = ' + targetTableAlias + '.' + key + ' + ' + escapeValue;
      }
      return targetTableAlias + '.' + key + ' = ' + escapeValue;
    }.bind(this)).join(', ');
  ...
  selectFromTableFragment: function(options, model, attributes, tables, mainTableAs, where) {
    ...
    if(options.with) {
      mainFragment += ' WITH (' + options.with.join(',') + ')';
    }
  ...
  addLimitAndOffset: function(options, model) {
    ...
    if (options.limit || options.offset) {
      if (!options.order || (options.include && !orders.subQueryOrder.length)) {
        ...
        fragment += options.field ? this.quoteIdentifier(options.field) : (this.quoteIdentifier(model.name) + "." + this.quoteIdentifier(model.primaryKeyField));
      }
