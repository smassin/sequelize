...
Instance.prototype.get = function(key, options) { // testhint options:none
  ...
  if (key) {
    ...
    if (options && options.plain && this.$options.include && this.$options.includeNames.indexOf(key) !== -1) {
      if(this.$options.includeMap[key].keyBy) {
        return _.mapValues(this.dataValues[key], function (instance) { return instance.get({plain: options.plain}); });
      } else if (Array.isArray(this.dataValues[key])) {
...
Instance.prototype._setInclude = function(key, originalValue, options) {
  ...
  if (include.originalAttributes === undefined || include.originalAttributes.length) {
    if (association.isSingleAssociation) {
      ...
      self[accessor] = isEmpty ? null : include.model.build(value, childOptions);
      if(include.isDataValue !== false) self.dataValues[accessor] = self[accessor];
      if(!isEmpty && _.isArray(include.applyAttributes)) self.set(_.pick(self[accessor].dataValues, include.applyAttributes), {raw:true})
    } else {
      isEmpty = value[0] && value[0][primaryKeyAttribute] === null;
      // Note: the _.values(_.mapValues()) is because when using a query with separate = true, the original value is from a .get() which returns a plain object maping to Instances rather than a plain object mapping to plain objects
      self[accessor] = isEmpty ? [] : include.model.bulkBuild(((include.keyBy && _.isPlainObject(originalValue)) ? _.values(_.mapValues(originalValue, function (v) { return (v instanceof Instance ? v.dataValues : v); })) : value), childOptions);

      if(include.keyBy) {
        self[accessor] = _.keyBy(self[accessor], include.keyBy);
      }
      if(include.isDataValue !== false) self.dataValues[accessor] = self[accessor];
    }
  }
      ...
      return self.sequelize.getQueryInterface()[query].apply(self.sequelize.getQueryInterface(), args)
        ...
        .tap(function() {
          ...
          return Promise.map(self.$options.include.filter(function (include) {
            ...
            // could also be if(self.$options.include.keyBy)
            if (_.isPlainObject(instances)) instances = _.values(instances);
