...
validateIncludedElement = function(include, tableNames, options) {
  ...
  if (include.separate === true && !(include.association instanceof HasMany) && !(include.association instanceof BelongsTo) && !(include.association instanceof HasOne)) {
    ...
    return include.association.get(results, _.assign(
      ...
      results.forEach(function (result) {
        result.set(
          ...
          map[result.get(include.association instanceof BelongsTo  ? include.association.foreignKey : include.association.source.primaryKeyAttribute)],
          {
            raw: true
          }
        );
...
Model.prototype.count = function(options) {
  ...
  return Promise.bind(this).then(function() {
    ...
    var col = options.include ? this.name + '.' + (options.field || this.primaryKeyField) : '*';
    options.field = 'count';
    ...
    // options.limit = limit !== undefined ? limit : null;
    // options.offset = offset !== undefined ? offset : null;
    // options.order = order !== undefined ? order : null;
...
Model.prototype.findOrInitialize = Model.prototype.findOrBuild = function(options) {
  ...
  return self.find(options).then(function(instance) {
    if (instance === null) {
      ...
      instance = self.build(values,options);
...
Model.prototype.upsert = function (values, options) {
  ...
  var buildValues = _.extend({}, values);
  var incrementValues = {};
  for(var i in buildValues) {
    var val = buildValues[i];
    if(val instanceof Utils.increment) {
      buildValues[i] = val.val;
  ...
  var createdAtAttr = this._timestampAttributes.createdAt
    ...
    , instance = this.build(buildValues);

  return instance.hookValidate(options).bind(this).then(function () {
    // Map field names
    var updatedDataValues = _.extend(_.pick(instance.dataValues, Object.keys(instance._changed)), incrementValues)
      , insertValues = Utils.mapValueFieldNames(instance.dataValues, instance.attributes, this)
